...
1. 输入以下提示：
    ...
    Copilot 会提供一份详细的重要要点总结，本质上是数据的执行摘要。 它展示了数据的模式和解释，并给出了推荐的后续步骤。 在此响应中，可以提示 Copilot 执行以下操作：

    +++创建一个表，其中显示数据中的键模式。+++

    Copilot 会创建一个表，其中包含另外一个显示键模式的列。
...
1. 查看 Copilot 的响应，其中包括图表和文本摘要，如果需要，可以将折线图添加到新工作表中。

1. 如果你添加了折线图，请查看图表，然后返回到工作表 1。
...
1. Copilot 显示条形图。  请注意，Copilot 还在响应中提供了文本见解。 选择“添加到新工作表”，在 Excel 工作簿中包含图表。****
