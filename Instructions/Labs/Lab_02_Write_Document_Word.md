...
1. 在浏览器中，从 [word.new](https://word.new) 打开一个新的 Word 文档。
...
1. 手动输入正斜杠后跟文档名称：**/Promotion Plan for Chai Tea in Latin America.docx**，以将文档引用添加到提示中。 或者，当文件在 OneDrive 中可用后，可以选择“添加内容”，然后从下拉列表中选择该文件。****
...
1. 要求 Copilot 使用画布上的 Copilot 选项+++添加项目里程碑及其截止时间的列表+++。 然后选择“生成”。****
...
1. 查看表格格式并进行任何必要的调整。 然后，可以将项目符号列表从“项目里程碑和截止时间”部分删除。
...
1. 查看生成的摘要，如果要在文档中包括整个摘要，请选择“添加到文档”（加号）****。 还可以突出显示文本的任何部分，然后将其复制并粘贴到文档中。

> [!IMPORTANT]
> Copilot 生成的文本将插入到文档中光标所在位置。 选择插入内容之前，请确保已导航到文档末尾。

可以手动调整文本，或突出显示摘要内容，并根据需要使用 Copilot 自动重写****。 与利益干系人共享项目报表时，还可以使用此摘要开启 Teams 或 Outlook 消息。
